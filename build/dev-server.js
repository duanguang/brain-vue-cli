...
const HISTORY_REWRITE_FALL_BACK_REGEX_FUNC = (name) => {
    ...
    const path = `${constants_1.URL_PREFIX}/${projectName}/${name}`;
...
var uri = `http://localhost:${port}/${constants_1.URL_PREFIX}/${name}/`;
var _resolve;
...
devMiddleware.waitUntilValid(() => {
    ...
    if (autoOpenBrowser && process.env.NODE_ENV !== 'testing') {
        if (!apps[0]) {
            //http://localhost:8000/app2/index.html#/
            console.warn(`忽略自动打开浏览器功能:`);
            console.warn(`请提供指定需要app name作为相对路径`);
        }
        else {
            uri = `${uri}${apps[0]}`;
            opn(uri);
